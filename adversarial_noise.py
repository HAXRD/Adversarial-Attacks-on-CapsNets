...
def BIM(loss, xs, batch_size, eps=0.01, epochs=1, clip_min=0., clip_max=1.):
    ...
    xs_adv_split = tf.split(xs_adv, num_or_size_split=batch_size, axis=0)

    loss_split = tf.split(loss, num_or_size_split=batch_size, axis=0)
    ...
    def _body(xs_adv_split, i):
        for k in range(batch_size):
            dy_dx, _ = tf.gradients(loss_split[k], xs_adv_split[k])
